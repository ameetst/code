"""
milt_nseall_backtest.py
=========================
MILT's current live rules (BB_WINDOW/BB_STD/MA_PERIOD/ATR_PERIOD/ATR_MULT/
STOP_LOSS_PCT/MAX_POSITIONS/POSITION_PCT -- read fresh from milt_strategy.py,
so this reflects whatever's currently configured, e.g. via MILT_config.json)
run on the much larger NSEAll universe (~2078 tickers vs N750's 750),
with THREE additional entry-only filters not present in the N750 backtest:

  1. SERIES == "EQ"   -- from ../Sharpe/Price_Band_List.csv (Symbol -> Series).
                         Excludes SME/BE/other non-mainboard-equity series.
                         Tickers missing from the list default to "EQ"
                         (matches momentum_lib.compute_universe_rankings'
                         existing convention).
  2. ADTV > Rs 1 crore -- Average Daily Traded Value (close x volume),
                         rolling 90-trading-day median, evaluated as of each
                         signal week. This is a liquidity floor -- NOT the
                         same window convention as Sharpe's compute_turnover
                         (12M/6M OR-eligibility); 90 days was chosen as a
                         reasonable middle ground for a strategy that reacts
                         to fresh weekly breakouts. Flagged as a judgment
                         call, not a stated rule.
  3. Price > Rs 100    -- excludes low-priced/penny stocks at entry.

These filters gate NEW ENTRIES only (matching how a real screen is normally
applied) -- they do not force an exit if a held position's price/turnover
later drops below the threshold; exits are still governed purely by MILT's
existing 3 exit rules.

Portfolio engine (sizing, max positions, tie-break, exits) is otherwise
identical to milt_backtest.py -- only the universe and the entry-eligibility
gate differ.

Usage
-----
    python milt_nseall_backtest.py [--file NSEAll_backtest.xlsx]
                                    [--adtv-threshold 10000000] [--min-price 100]
"""

import argparse
import sys
from pathlib import Path

import numpy as np
import pandas as pd

BACKTEST_DIR = Path(__file__).resolve().parent
MAIN_DIR = BACKTEST_DIR.parent
SHARPE_DIR = MAIN_DIR.parent / "Sharpe"
if str(MAIN_DIR) not in sys.path:
    sys.path.insert(0, str(MAIN_DIR))

import momentum_lib as ml
from milt_strategy import (
    BB_WINDOW, BB_STD, MA_PERIOD, ATR_PERIOD, ATR_MULT,
    STOP_LOSS_PCT, MAX_POSITIONS, POSITION_PCT,
)
from milt_backtest import (
    screen_split_artifacts, _next_available,
    compute_metrics, compute_benchmark_metrics, trade_stats,
    DEFAULT_CAPITAL,
)

DEFAULT_FILE = str(BACKTEST_DIR / "NSEAll_backtest.xlsx")
BAND_CSV = SHARPE_DIR / "Price_Band_List.csv"
ADTV_WINDOW_DAYS = 90


def load_series_map(csv_path: Path) -> dict:
    series_map = {}
    if csv_path.exists():
        try:
            df = pd.read_csv(csv_path)
            for _, r in df.iterrows():
                series_map[str(r["Symbol"]).strip()] = str(r["Series"]).strip()
        except Exception as e:
            print(f"  Warning: could not read {csv_path}: {e}")
    return series_map


def build_all_weekly_filtered(ohlc: dict, exclude: set = None) -> dict:
    """Weekly OHLC + MILT indicators + a weekly-aligned ADTV (Rs) series."""
    exclude = exclude or set()
    weekly = {}
    for t in ohlc["tickers"]:
        if t in exclude:
            continue
        if any(ohlc[f] is None or t not in ohlc[f].index for f in ("open", "high", "low", "close")):
            continue
        wdf = ml.build_weekly_indicators(
            ohlc, t, bb_window=BB_WINDOW, bb_std=BB_STD,
            ma_period=MA_PERIOD, atr_period=ATR_PERIOD,
        )
        if wdf is None or len(wdf) < BB_WINDOW + 1:
            continue
        wdf = wdf.copy()
        wdf["roc_12m"] = wdf["close"] / wdf["close"].shift(52) - 1.0

        # -- weekly-aligned ADTV (Rs), rolling 90-trading-day median of close*volume --
        if ohlc["volume"] is not None and t in ohlc["volume"].index:
            close_d = ohlc["close"].loc[t]
            vol_d = ohlc["volume"].loc[t]
            common = close_d.index.intersection(vol_d.index)
            turnover_d = (close_d[common] * vol_d[common]).dropna()
            turnover_d.index = pd.to_datetime(turnover_d.index)
            turnover_d = turnover_d.sort_index()
            adtv_d = turnover_d.rolling(ADTV_WINDOW_DAYS, min_periods=20).median()
            # reindex onto the weekly bar dates: value as of (on/before) that Friday
            adtv_w = adtv_d.reindex(adtv_d.index.union(wdf.index)).sort_index().ffill().reindex(wdf.index)
            wdf["adtv"] = adtv_w
        else:
            wdf["adtv"] = np.nan

        weekly[t] = wdf
    return weekly


def run_backtest(weekly: dict, series_map: dict, capital: float,
                 adtv_threshold: float, min_price: float):
    master_dates = sorted(set().union(*[set(df.index) for df in weekly.values()]))
    warmup = MA_PERIOD
    if len(master_dates) <= warmup + 1:
        raise ValueError("Not enough weekly history to backtest.")

    cash = capital
    positions: dict[str, dict] = {}
    trades = []
    equity_curve = []

    for i in range(warmup, len(master_dates) - 1):
        date = master_dates[i]

        for t, pos in positions.items():
            wdf = weekly.get(t)
            px = wdf.loc[date, "close"] if (wdf is not None and date in wdf.index) else np.nan
            pos["last_price"] = px if pd.notna(px) else pos["last_price"]

        # ── exits (identical to MILT) ───────────────────────────────────────
        exit_tickers = []
        for t, pos in list(positions.items()):
            wdf = weekly.get(t)
            if wdf is None or date not in wdf.index:
                continue
            bar = wdf.loc[date]
            close = bar["close"]
            if pd.isna(close):
                continue
            pos["peak_close"] = max(pos["peak_close"], close)

            reason = None
            if close <= pos["entry_price"] * (1 - STOP_LOSS_PCT):
                reason = "stop_loss_20pct"
            elif pd.notna(bar["sma23"]) and close < bar["sma23"]:
                reason = "trend_reversal_23wma"
            elif pd.notna(bar["atr"]) and close < pos["peak_close"] - ATR_MULT * bar["atr"]:
                reason = "atr_chandelier_stop"

            if reason:
                exit_tickers.append((t, reason))

        for t, reason in exit_tickers:
            wdf = weekly[t]
            exec_date, exec_price = _next_available(wdf, date)
            if exec_date is None:
                continue
            pos = positions.pop(t)
            proceeds = pos["shares"] * exec_price
            cash += proceeds
            hold_weeks = master_dates.index(exec_date) - master_dates.index(pos["signal_date"])
            trades.append({
                "ticker": t, "reason": reason,
                "entry_date": pos["entry_date"].isoformat(), "entry_price": pos["entry_price"],
                "exit_date": exec_date.date().isoformat(), "exit_price": exec_price,
                "shares": pos["shares"], "hold_weeks": hold_weeks,
                "pnl_pct": round((exec_price / pos["entry_price"] - 1) * 100, 2),
                "pnl_rs": round(proceeds - pos["shares"] * pos["entry_price"], 2),
            })

        # ── entries: MILT breakout AND series==EQ AND ADTV AND min price ───────
        available_slots = MAX_POSITIONS - len(positions)
        if available_slots > 0:
            candidates = []
            for t, wdf in weekly.items():
                if t in positions or date not in wdf.index:
                    continue
                bar = wdf.loc[date]
                if pd.isna(bar["bb_upper"]) or pd.isna(bar["close"]):
                    continue
                if bar["close"] <= bar["bb_upper"]:
                    continue
                if series_map.get(t, "EQ") != "EQ":
                    continue
                if bar["close"] <= min_price:
                    continue
                if pd.isna(bar["adtv"]) or bar["adtv"] <= adtv_threshold:
                    continue
                candidates.append((t, bar["close"], bar["roc_12m"]))

            candidates.sort(key=lambda c: (c[2] if pd.notna(c[2]) else -999), reverse=True)
            selected = candidates[:available_slots]

            equity_after_exits = cash + sum(p["shares"] * p["last_price"] for p in positions.values())
            alloc = equity_after_exits * POSITION_PCT

            for t, sig_close, roc in selected:
                wdf = weekly[t]
                exec_date, exec_price = _next_available(wdf, date)
                if exec_date is None or exec_price <= 0:
                    continue
                shares = int(alloc // exec_price)
                if shares <= 0:
                    continue
                cost = shares * exec_price
                if cost > cash:
                    continue
                cash -= cost
                positions[t] = {
                    "entry_date": exec_date.date(), "entry_price": exec_price,
                    "shares": shares, "peak_close": exec_price,
                    "last_price": exec_price, "signal_date": date,
                }

        pos_value = sum(p["shares"] * p["last_price"] for p in positions.values())
        nav = cash + pos_value
        equity_curve.append({
            "date": date.date().isoformat(), "nav": nav,
            "n_positions": len(positions),
            "invested_frac": (nav - cash) / nav if nav > 0 else 0.0,
        })

    for t, pos in positions.items():
        final_price = pos["last_price"]
        proceeds = pos["shares"] * final_price
        hold_weeks = len(master_dates) - 1 - master_dates.index(pos["signal_date"])
        trades.append({
            "ticker": t, "reason": "open_at_backtest_end",
            "entry_date": pos["entry_date"].isoformat(), "entry_price": pos["entry_price"],
            "exit_date": master_dates[-1].date().isoformat(), "exit_price": final_price,
            "shares": pos["shares"], "hold_weeks": hold_weeks,
            "pnl_pct": round((final_price / pos["entry_price"] - 1) * 100, 2),
            "pnl_rs": round(proceeds - pos["shares"] * pos["entry_price"], 2),
        })

    return pd.DataFrame(equity_curve), pd.DataFrame(trades), master_dates


def main():
    parser = argparse.ArgumentParser(description="MILT on NSEAll with SERIES/ADTV/price filters.")
    parser.add_argument("--file", default=DEFAULT_FILE)
    parser.add_argument("--capital", type=float, default=DEFAULT_CAPITAL)
    parser.add_argument("--adtv-threshold", type=float, default=10_000_000,
                        help="Minimum ADTV in Rs (default: Rs 1 crore)")
    parser.add_argument("--min-price", type=float, default=100.0)
    args = parser.parse_args()

    if not Path(args.file).exists():
        print(f"ERROR: {args.file} not found. Run milt_update_prices.py "
              f"--tickers-from ../Sharpe/NSEAll.xlsx --output {args.file} --period 5y first.")
        return

    print(f"Loading {args.file} ...")
    ohlc = ml.load_ohlc(args.file)
    print(f"  {len(ohlc['tickers'])} tickers | {len(ohlc['dates'])} daily bars "
          f"({ohlc['dates'][0]} -> {ohlc['dates'][-1]})\n")

    print(f"Loading SERIES data from {BAND_CSV} ...")
    series_map = load_series_map(BAND_CSV)
    n_eq = sum(1 for v in series_map.values() if v == "EQ")
    print(f"  {len(series_map)} tickers mapped ({n_eq} EQ); "
          f"tickers not in this list default to EQ\n")

    flagged = screen_split_artifacts(ohlc)
    print(f"Excluded {len(flagged)} split-artifact ticker(s)\n")

    print(f"Building weekly indicators (BB{BB_WINDOW}/{BB_STD}sigma, "
          f"MA{MA_PERIOD}w, ATR{ATR_PERIOD}w) + {ADTV_WINDOW_DAYS}d rolling ADTV ...")
    weekly = build_all_weekly_filtered(ohlc, exclude=flagged)
    print(f"  {len(weekly)} tickers with usable weekly history\n")

    print(f"Running walk-forward weekly backtest "
          f"(SERIES=EQ, ADTV>Rs{args.adtv_threshold:,.0f}, price>Rs{args.min_price:.0f}) ...")
    equity_df, trades_df, master_dates = run_backtest(
        weekly, series_map, args.capital, args.adtv_threshold, args.min_price)
    print(f"  Simulated {len(equity_df)} weeks "
          f"({equity_df['date'].iloc[0]} -> {equity_df['date'].iloc[-1]})")
    print(f"  {len(trades_df)} closed trades\n")

    equity_csv = BACKTEST_DIR / "milt_nseall_equity.csv"
    trades_csv = BACKTEST_DIR / "milt_nseall_trades.csv"
    equity_df.to_csv(equity_csv, index=False)
    trades_df.to_csv(trades_csv, index=False)
    print(f"Saved {equity_csv.name}, {trades_csv.name}\n")

    metrics = compute_metrics(equity_df, args.capital)
    tstats = trade_stats(trades_df)
    bench = compute_benchmark_metrics(
        ohlc["nifty"], equity_df["date"].iloc[0], equity_df["date"].iloc[-1], args.capital)

    print(f"{'='*70}\nMILT on NSEAll (SERIES=EQ, ADTV>{args.adtv_threshold:,.0f}, "
          f"price>{args.min_price:.0f}) -- RESULTS\n{'='*70}")
    print(f"  Period            : {metrics['start_date']} -> {metrics['end_date']} ({metrics['years']} yrs)")
    print(f"  CAGR              : {metrics['cagr_pct']:+.2f}%")
    print(f"  Annualised vol    : {metrics['ann_vol_pct']:.2f}%")
    print(f"  Sharpe (rf 7%)    : {metrics['sharpe']}")
    print(f"  Max drawdown      : {metrics['max_drawdown_pct']:.2f}%")
    print(f"  Trades            : {tstats['n_trades']}")
    if tstats["n_trades"]:
        print(f"  Win rate          : {tstats['win_rate_pct']}%")
        print(f"  Avg win / loss    : {tstats['avg_win_pct']:+.2f}% / {tstats['avg_loss_pct']:+.2f}%")
        print(f"  Best / worst trade: {tstats['best_trade_pct']:+.2f}% / {tstats['worst_trade_pct']:+.2f}%")
    if bench:
        print(f"  NIFTY500 CAGR (same window): {bench['cagr_pct']:+.2f}%")
    print(f"{'='*70}")


if __name__ == "__main__":
    main()
