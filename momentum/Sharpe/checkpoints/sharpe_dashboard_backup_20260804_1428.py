...
def append_score_history(universe_name, input_path, result_df):
    ...
    replaces that day's entry rather than appending a duplicate, so the
    Early Movers trend window (last 5 entries) always spans distinct trading
    days instead of being diluted by same-day reruns.
...
with tab_calcs:
    ...
    for c in ["RES_MOM", "1M%", "3M%", "12M%", "PCT_FROM_52H", "REL_52H_DD"]:
