...
def load_cash_ledger(universe_name):
    path = SCRIPT_DIR / f"{universe_name}_cash_ledger.json"
    if not path.exists():
        try:
            safe_write_json(path, [])
        except Exception as e:
            st.error(f"Error initializing empty cash ledger: {e}")
        return []
    try:
        with open(path, "r") as f:
            return json.load(f)
    except Exception as e:
        st.error(f"Error loading cash ledger: {e}")
        bak_path = path.with_suffix(".bak")
        if bak_path.exists():
            st.warning("Attempting recovery from backup file...")
            try:
...
def save_cash_ledger(universe_name, cash_ledger):
    path = SCRIPT_DIR / f"{universe_name}_cash_ledger.json"
    try:
        safe_write_json(path, cash_ledger)
    except Exception as e:
...
def compute_cash_ledger_running_balance(cash_ledger):
    """Replay entries chronologically and return them with a running balance,
    oldest-first."""
    try:
        sorted_entries = sorted(cash_ledger, key=lambda x: (x.get("date", ""), x.get("timestamp", "")))
    except Exception:
        sorted_entries = cash_ledger
    running = 0.0
    out = []
...
def append_score_history(universe_name, input_path, result_df):
    ...
    for ticker in result_df.index:
        ...
        z3m_val = result_df.get("SHARPE_3", pd.Series(dtype=float)).get(ticker)
...
def compute_rising_candidates(history, result_df, held_tickers):
    ...
    Also computes Z_3M velocity and streak from z3m_scores, add columns to output.
    ...
    for ticker in all_tickers:
        ...
        # Z_3M velocity and streak (may have fewer data points)
...
# Load volume data (optional — returns None if VOLUME sheet absent)
try:
    volume_df = load_volume_data(input_path)
except Exception:
    volume_df = None
        ...
        if n_new_positions > 0 and allow_new:
            ...
            if not entry_candidates:
                ...
                if available_cash <= 0:
                    ...
                    # Circuit-hit frequency (UC/LC over trailing 252 sessions) — always shown
                    # here regardless of the Circuit Hit Frequency Filter setting.
                    try:
                        entry_circuit_df = ml.compute_circuit_hits(
                            prices_df, entry_candidates, str(band_csv), lookback_period=252)
                    except Exception:
                        entry_circuit_df = None

                    entry_rows = []
                    for t in entry_candidates:
                        if t not in entry_weights:
                            continue
                        inv_amount = min(entry_weights[t] * capital * scale, available_cash)
                        ...
                        if entry_circuit_df is not None and t in entry_circuit_df.index:
                            uc_hits = int(entry_circuit_df.loc[t, "UC_COUNT"])
                            lc_hits = int(entry_circuit_df.loc[t, "LC_COUNT"])
                            circuit_hits_str = f"{uc_hits} / {lc_hits}"
                        else:
                            circuit_hits_str = "—"
...
with tab_early:
    ...
    if n_hist_runs < 3:
        ...
        st.caption(
            ...
            "White = 1 run  |  "
            "Velocity = composite score slope (last 5 runs)  |  "
            "Z3M Vel = 3-month Z-score slope (early signal)")
