...
unchanged). Ranks stocks by a new blended score:

    BLEND = 0.5 * z(SHARPE_COMPOSITE_raw) + 0.5 * z(CLENOW_RAW)

- SHARPE_COMPOSITE_raw = mean(Z_12M, Z_9M, Z_6M, Z_3M) from ml.compute_sharpe,
  BEFORE the normalise_composite() rescale (so it's a plain Z-score average).
- CLENOW_RAW = the ACTUAL Clenow Score, computed by re-using
  `Clenow.clenow_score()` from the existing
  C:\\...\\momentum\\Clenow Score\\Clenow.py tool (not reinvented here):
  a 90-day (MOMENTUM_WINDOW) log-price exponential regression, annualised
  as (exp(slope*250)-1)*100 (TRADING_DAYS_PER_YEAR=250, book convention),
  multiplied by the fit's R^2. One number per stock per rebalance date, no
  cross-sectional Z-scoring inside Clenow.py itself (its own rank() just
  sorts by this raw score).
- Both raw scores are re-standardised cross-sectionally (ml._cross_section_z)
  right before blending, so a 50/50 weight is a true equal contribution
  regardless of each raw score's own scale/variance (COMPOSITE is already a
  Z-score average; the Clenow score is a %-annualised-return-like number in
  the tens, so blending it in unstandardised would let it dominate purely on
  units).
- The blend is then passed through the same normalise_composite() used for
  the live COMPOSITE, and used for: eligibility ranking, the RANK<=40 hold
  buffer, and volatility-adjusted position weighting.

Everything else (52H eligibility filter, regime score / dynamic N, entry
buffer, exit rules, vol-sizing formula, friction) is IDENTICAL to
backtest_wired.py, and the regime score is still fed the ORIGINAL Sharpe
COMPOSITE (not the blend) so this test isolates the effect of the new
stock-selection score, holding market-timing logic constant.

Note: only Clenow.py's SCORE FORMULA is reused (exp_regression /
clenow_score) — the rest of Clenow.py's own strategy machinery (ATR sizing,
chandelier stops, 100-day MA filter, 15% gap disqualifier, liquidity filter,
20%-band hold logic) is a separate, self-contained trading system and is NOT
pulled in here; this backtest's eligibility/exit rules remain the Sharpe
strategy's own (52-week-high filter, dynamic regime, RANK<=40 buffer).
"""
import sys
import os
import datetime
...
# Make momentum_lib importable regardless of cwd (this script lives one
# level below the Sharpe root, where momentum_lib.py actually resides).
SHARPE_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, SHARPE_ROOT)
import momentum_lib as ml

# Reuse the ACTUAL Clenow Score tool's scoring function instead of
# reimplementing it — per user instruction.
CLENOW_SCORE_DIR = os.path.join(os.path.dirname(SHARPE_ROOT), "Clenow Score")
sys.path.insert(0, CLENOW_SCORE_DIR)
import Clenow as cl
...
W_SHARPE = float(sys.argv[1]) if len(sys.argv) > 1 else 0.5
W_CLENOW = float(sys.argv[2]) if len(sys.argv) > 2 else 0.5
WEIGHT_TAG = f"{int(round(W_SHARPE*100))}s{int(round(W_CLENOW*100))}c"
...
print(f"\nStarting Point-in-Time Vectorised Backtest (Sharpe/Clenow {W_SHARPE:.0%}/{W_CLENOW:.0%} blend):")
print("-" * 80)
...
for i in range(len(valid_dates) - 1):
    ...
    # Original Sharpe COMPOSITE (raw mean-of-Z, then normalised) — used ONLY
    # for the regime score, exactly as in backtest_wired.py.
    core_labels = [l for l in SHARPE_WINDOWS if l != "1M"]
    z_cols = [f"Z_{l}" for l in core_labels]
    sharpe_composite_raw = z_df[z_cols].mean(axis=1)
    result["COMPOSITE"] = sharpe_composite_raw.map(ml.normalise_composite)

    # Real Clenow Score, point-in-time: Clenow.py's own clenow_score() on the
    # trailing MOMENTUM_WINDOW (90) days of this ticker's price history as
    # of t_date — identical formula the live Clenow Score tool uses.
    clenow_scores = {}
    for ticker in stock_tickers:
        px = sliced_prices.loc[ticker].dropna()
        px = px[px > 0]
        if len(px) < cl.MOMENTUM_WINDOW:
            continue
        recent = px.iloc[-cl.MOMENTUM_WINDOW:]
        score, _slope, _r2 = cl.clenow_score(recent.values)
        if not np.isnan(score):
            clenow_scores[ticker] = score
    clenow_raw = pd.Series(clenow_scores).reindex(result.index)

    # Re-standardise both raw scores before blending so 50/50 is a true
    # equal-variance contribution, not skewed by whichever is naturally
    # more dispersed. Missing Clenow scores (insufficient history) fall back
    # to the universe average (Z=0), same missing-data convention momentum_lib
    # itself uses for COMPOSITE.
    sharpe_std = ml._cross_section_z(sharpe_composite_raw)
    clenow_std = ml._cross_section_z(clenow_raw.fillna(clenow_raw.mean()))
    clenow_std = clenow_std.fillna(0.0)

    blend_raw = W_SHARPE * sharpe_std + W_CLENOW * clenow_std
    result["BLEND"] = blend_raw.map(ml.normalise_composite)

    # ── FILTER AND RANK (by BLEND instead of COMPOSITE) ───────────────
    eligible_mask = result["PCT_FROM_52H"] >= -25
    elig_df = result[eligible_mask].copy()
    elig_df["RANK"] = elig_df["BLEND"].rank(ascending=False, method="first", na_option="bottom")
    elig_df = elig_df.sort_values("RANK", ascending=True)
    top_candidates = elig_df.index.tolist()

    # Regime score still driven by the ORIGINAL Sharpe COMPOSITE — keeps
    # market-timing / dynamic-N behaviour identical to the baseline so this
    # test isolates the effect of the new stock-selection score.
    with suppress_stdout():
    ...
    for ticker in next_portfolio_tickers:
        comp_score = result.loc[ticker, "BLEND"]
        px = sliced_prices.loc[ticker].dropna()
        if len(px) > 10:
            vols = []
...
run_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"run_blend_{WEIGHT_TAG}_{timestamp}")
...
print(f"\n=== PERFORMANCE SUMMARY ({W_SHARPE:.0%} Sharpe COMPOSITE / {W_CLENOW:.0%} Clenow) ===")
...
try:
    ...
    plt.title(f'{W_SHARPE:.0%} Sharpe COMPOSITE / {W_CLENOW:.0%} Clenow Blend vs NIFTY500\n(Red shading = CASH Regime)')
    plt.xlabel('Date')
