"""
backtest_resmom_gt1_nseall.py
==============================
Sharpe momentum strategy backtest (dynamic regime engine, same mechanics as
backtest.py) with a RES_MOM > 1.0 new-entry confirmation filter, run over
the full NSEAll universe using the 10-year history file:

    C:\\Users\\ameet\\Documents\\Github\\dhan_datahq\\base files\\History_updated_NSEAll.xlsx

No prior run in this repo tested a RES_MOM > 1 gate (the only precedent,
sandbox_residual_winsorise.py, tested RES_MOM > 0). This script mirrors
that pattern but:
  - raises the confirmation threshold from 0 to 1.0
  - uses momentum_lib's production compute_residual_momentum (same function
    Sharpe.py uses live) instead of a bespoke OLS/winsorisation routine
  - runs on the full NSEAll universe (~2,077 tickers) instead of N500

RES_MOM is a new-entry confirmation gate only: existing holdings are never
forced out for failing it (matches the hysteresis design — avoids whipsaw).
"""
...
RESMOM_THRESHOLD = 1.0      # <-- the filter under test
...
print(f"RES_MOM new-entry confirmation threshold: > {RESMOM_THRESHOLD}")
...
for i in range(len(valid_dates) - 1):
    ...
    with suppress_stdout():
        ...
        _, rs_z_df = ml.compute_residual_momentum(
            sliced_prices, stock_tickers, sliced_nifty, SHARPE_WINDOWS, TRADING_DAYS, rfr_daily)

    result = z_df.copy()
    result["PCT_FROM_52H"] = pct_52h
    result["RES_MOM"] = rs_z_df["RES_MOM"]
    ...
    # Pass 1: retain held stocks (exit evaluation always runs; RES_MOM not applied on exit)
    ...
    # Pass 2: fill new slots, gated by regime AND RES_MOM > threshold
    if allow_new:
        slots_to_fill = dynamic_n - len(next_portfolio_tickers)
        for ticker in top_candidates:
            if slots_to_fill <= 0:
                break
            if ticker in next_portfolio_tickers:
                continue
            resmom_val = result.loc[ticker, "RES_MOM"]
            if pd.isna(resmom_val) or resmom_val <= RESMOM_THRESHOLD:
                continue
            next_portfolio_tickers.append(ticker)
            slots_to_fill -= 1
    ...
    sys.stdout.write(f"\r  [{i+1}/{len(valid_dates)-1}] {t_date.strftime('%b %Y')} | "
                     ...
                     f"Held: {len(actual_portfolio):2d} | elapsed: {elapsed:6.0f}s")
    sys.stdout.flush()
...
run_dir = os.path.join(SHARPE_ROOT, "backtest results", f"run_{timestamp}_resmom_gt1_nseall")
...
print("\n=== PERFORMANCE SUMMARY ===")
print(f"Universe: NSEAll ({len(stock_tickers)} tickers)  |  RES_MOM > {RESMOM_THRESHOLD} entry filter")
...
try:
    ...
    plt.plot(df_res['Rebalance_Date'], df_res['Equity'], label=f"Strategy RES_MOM>{RESMOM_THRESHOLD} (CAGR {p_cagr:.1f}%)", color='#0055CC', linewidth=2)
    ...
    plt.title(f'Sharpe Vol-Sized Strategy (RES_MOM > {RESMOM_THRESHOLD} filter, NSEAll) vs NIFTY500\n(Red shading = CASH Regime)')
    plt.xlabel('Date')
...
try:
    shutil.copy2(__file__, os.path.join(run_dir, "backtest_resmom_gt1_nseall.py"))
except Exception:
    pass
