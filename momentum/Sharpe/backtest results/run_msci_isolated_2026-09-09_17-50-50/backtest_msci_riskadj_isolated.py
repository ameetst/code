...
unchanged). Variant of backtest_msci_riskadj.py that ISOLATES the ranking
methodology: the MSCI-style Risk-Adjusted Momentum score (RAM) is used for
stock selection / RANK<=40 hold buffer / vol-adjusted position weighting,
but the dynamic regime score (market-timing, cash/BUY, dynamic N) is fed the
ORIGINAL Sharpe COMPOSITE, exactly as backtest_wired.py computes it —
identical to how the earlier Sharpe/Clenow blend tests were isolated. This
makes it a clean head-to-head of RANKING METHODOLOGY ONLY (RAM vs COMPOSITE
for picking/weighting stocks), with market-timing held constant so any
difference in results traces purely to which stocks get selected, not to
different regime/cash behaviour.

SCORE DEFINITION (per the spec: "6- and 12-month returns over T-bills, each
divided by the stock's 3-year weekly volatility, then averaged") — unchanged
from backtest_msci_riskadj.py:
  ...
  - vol_3y uses TRUE calendar week-end closes; needs >= 104 weeks (2y) of
    history, else RAM falls back to that date's cross-sectional mean.
  - RAM is passed through ml.normalise_composite() before ranking/weighting
    only so it behaves like COMPOSITE downstream (stays positive for
    vol-adjusted position sizing); this is a monotonic transform and does
    not change rank order.

Everything else — 52-week-high eligibility filter, RANK<=40 hold buffer /
28-day min-hold, volatility-adjusted position sizing, 5% cap, friction — is
IDENTICAL machinery to backtest_wired.py, reused as-is via momentum_lib.
The ONLY thing that differs from backtest_wired.py is which score ranks and
weights the portfolio (RAM here vs COMPOSITE there); regime/timing is
...
# ── VECTORISED, BACKWARD-LOOKING-ONLY PRECOMPUTATION (RAM score inputs) ───────
...
print("\nStarting Point-in-Time Vectorised Backtest (MSCI RAM ranking, regime held on Sharpe COMPOSITE):")
print("-" * 80)
...
for i in range(len(valid_dates) - 1):
    ...
    with suppress_stdout():
        ...
        # Only computed to drive the regime score identically to
        # backtest_wired.py — held fixed so this test isolates the ranking
        # methodology (RAM vs COMPOSITE), not market-timing behaviour.
    ...
    sharpe_composite = sharpe_composite_raw.map(ml.normalise_composite)
    ...
    result["COMPOSITE"] = sharpe_composite
    ...
    # Regime score held fixed on the ORIGINAL Sharpe COMPOSITE — identical
    # call backtest_wired.py makes. This is the isolation: only the ranking
    # score (RAM vs COMPOSITE) differs between this script and the baseline.
    with suppress_stdout():
...
run_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"run_msci_isolated_{timestamp}")
...
print("\n=== PERFORMANCE SUMMARY (MSCI RAM ranking, regime held on Sharpe COMPOSITE) ===")
...
try:
    ...
    plt.title('MSCI RAM Ranking (regime on Sharpe COMPOSITE) vs NIFTY500\n(Red shading = CASH Regime)')
    plt.xlabel('Date')
