...
# ── DYNAMIC REGIME PARAMETERS (passed straight into ml.compute_regime_score,
#    the same single-source-of-truth function Sharpe.py uses live) ────────────
MIN_N               = 5
MAX_N               = 25
NEW_ENTRY_THRESHOLD = 0.35
DYN_N_FULL_SCORE    = 0.66   # regime score at which dynamic_n saturates at MAX_N
SIGNAL_WEIGHTS      = None   # None => ml.DEFAULT_SIGNAL_WEIGHTS (35/25/25/15)
...
for i in range(len(valid_dates) - 1):
    ...
    # Calculate dynamic regime score — wired directly to momentum_lib's
    # single-source-of-truth function (same one Sharpe.py calls live).
    with suppress_stdout():
...
run_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"run_{timestamp}")
...
try:
    ...
    plt.title('Sharpe Vol-Sized Strategy vs NIFTY500 (wired to ml.compute_regime_score)\n(Red shading = CASH Regime)')
    plt.xlabel('Date')
