...
unchanged). Combines all three regime-config changes tested individually in
prior runs, applied together on top of the plain-Sharpe COMPOSITE ranking
strategy (backtest_wired.py):

  1. Equal-weighted regime signals: 25/25/25/25 instead of the live default
     35% EMA50 breadth / 25% EMA trend breadth / 25% 52H breadth / 15% Sharpe
     momentum breadth (ml.DEFAULT_SIGNAL_WEIGHTS).
  2. Momentum breadth threshold = 1.0 instead of the live hardcoded 1.5
     (momentum_lib.py:1190, Signal 4 of ml.compute_regime_score).
  3. NEW_ENTRY_THRESHOLD = 0.35 and DYN_N_FULL_SCORE = 0.66, instead of the
     live defaults 0.40 / 0.75 (ml.DEFAULT_NEW_ENTRY_THRESHOLD /
     ml.DEFAULT_DYN_N_FULL_SCORE) — matches the repo's existing
     backtest_wired_035_066.py / backtest_variant_035_066.py precedent.

Individually, each of the three changes underperformed the live regime
config in isolation (see run_regime_eqweight_*, run_regime_thresh1_*). This
script tests whether stacking all three together behaves differently.

Ranking, 52H eligibility filter, dynamic-N mechanics, RANK<=40 hold buffer,
vol-adjusted weighting, friction — unchanged plain-Sharpe strategy machinery,
identical to backtest_wired.py. Only the regime score's internals differ.
"""
import sys
import os
import datetime
...
SIGNAL_WEIGHTS = {           # live default: 35/25/25/15
    "ema50_breadth":     0.25,
...
print(f"\nStarting Point-in-Time Vectorised Backtest "
      f"(Combined: eq-weight signals + threshold={MOMENTUM_THRESHOLD} + "
      f"entry={NEW_ENTRY_THRESHOLD}/full={DYN_N_FULL_SCORE}):")
print("-" * 80)
...
run_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"run_regime_combined_{timestamp}")
...
print(f"\n=== PERFORMANCE SUMMARY (Combined regime changes) ===")
...
try:
    ...
    plt.title('Combined Regime Changes vs NIFTY500\n(Red shading = CASH Regime)')
    plt.xlabel('Date')
