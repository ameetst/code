...
regime score's 4 signal weights are equal (25/25/25/25) instead of the live
default (35% EMA50 breadth / 25% EMA trend breadth / 25% 52H breadth / 15%
Sharpe momentum breadth) — see ml.compute_regime_score /
ml.DEFAULT_SIGNAL_WEIGHTS in momentum_lib.py. Everything else (ranking by
Sharpe COMPOSITE, 52H eligibility filter, dynamic N, hold buffer, vol-sized
weighting, friction) is the plain-Sharpe strategy, unchanged.
"""
import sys
import os
import datetime
...
# The only change vs backtest_wired.py: equal-weighted signals instead of
# ml.DEFAULT_SIGNAL_WEIGHTS (35/25/25/15).
SIGNAL_WEIGHTS = {
    "ema50_breadth":     0.25,
...
print(f"\nStarting Point-in-Time Vectorised Backtest (Equal-Weighted Regime Signals {SIGNAL_WEIGHTS}):")
print("-" * 80)
...
run_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"run_regime_eqweight_{timestamp}")
...
print("\n=== PERFORMANCE SUMMARY (Equal-Weighted Regime Signals 25/25/25/25) ===")
