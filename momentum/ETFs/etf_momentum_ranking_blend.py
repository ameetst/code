"""
ETF Dual Momentum + Sharpe/Clenow BLEND Scoring + Tiered Regime Filter
========================================================================
VARIANT of etf_momentum_ranking.py -- scoring formula only. Everything
else (screen, regime, hold-and-replace rebalance, exit triggers, sector
cap, holdings log, TSL check, Excel layout) is identical to the live
script. Writes to its own output/log files (see CONFIG) so it never
collides with the live script's state.
...
Scoring pipeline (in correct order):
  ...
  Step 2 - SCORE   : Compute TWO independent composites among screen-pass ETFs:
                       Sharpe composite  = 0.5*Z(Sharpe 6M) + 0.5*Z(Sharpe 3M)
                       Clenow composite  = 0.5*Z(Clenow 6M) + 0.5*Z(Clenow 3M)
                         (Clenow = Annualised log-price regression slope x R^2,
                          same formula as ../../Sharpe/momentum_lib.py::compute_clenow)
                     Final blended score = 0.5*Sharpe composite + 0.5*Clenow composite
                     (BLEND_W_SHARPE / BLEND_W_CLENOW in CONFIG).
                     Investable rank (RANK_INVESTABLE) is this blended score,
                     ranked within the screened subset only.
...
Backtest rationale (2020-04 to 2026-07, see ../backtest results/etf_backtest_v7_clenow_scoring.py):
  vs plain Sharpe-only ranking, the blend improved CAGR (16.51% -> 19.08%),
  max drawdown (-20.82% -> -19.22%) and cut rank-churn exits (134 -> 101),
  at the cost of higher volatility (12.13% -> 14.17%) and more TSL hits
  (34 -> 53) -- Clenow tends to favour steeper, smoother single-sector
  trends (PSU Bank, IT, Auto, Private Bank) over the broad low-vol baskets
  Sharpe alone favours (Sensex, Nifty100, Midcap-broad, Consumption, Gold).
...
# =========================================================
# CONFIG  <- loaded from strategy_config_blend.json if available,
#            otherwise uses hardcoded defaults below.
#            Deliberately a SEPARATE config file from the live script's
#            strategy_config.json so edits to one never leak into the other.
# =========================================================
_SCRIPT_DIR = Path(__file__).resolve().parent
_CONFIG_FILE = "strategy_config_blend.json"
...
def _apply_json_config(cls, cfg: dict):
    ...
    _KEYS = [
        ...
        "CLENOW_W6M", "CLENOW_W3M", "BLEND_W_SHARPE", "BLEND_W_CLENOW",
...
def get_config_as_dict() -> dict:
    ...
    return {
        ...
        "CLENOW_W6M": CONFIG.CLENOW_W6M,
        "CLENOW_W3M": CONFIG.CLENOW_W3M,
        "BLEND_W_SHARPE": CONFIG.BLEND_W_SHARPE,
        "BLEND_W_CLENOW": CONFIG.BLEND_W_CLENOW,
...
class CONFIG:
    INPUT_FILE  = "ETF.xlsx"
    OUTPUT_FILE = "etf_rankings_blend.xlsx"
    ...
    # Weighted Clenow blending (Annualised log-price regression slope x R^2,
    # same formula as ../../Sharpe/momentum_lib.py::compute_clenow). Rewards
    # steep AND smooth trends; R^2 penalises choppy movers.
    CLENOW_W6M  = 0.5
    CLENOW_W3M  = 0.5

    # Final composite = BLEND_W_SHARPE * SharpeComposite + BLEND_W_CLENOW * ClenowComposite
    # Backtested 2020-04 to 2026-07: improves CAGR/max-DD over Sharpe-only
    # at the cost of higher volatility. See module docstring for numbers.
    BLEND_W_SHARPE = 0.5
    BLEND_W_CLENOW = 0.5
...
def clenow_score(series: pd.Series, window: int, trading_days: int | None = None) -> float:
    """
    Clenow momentum score: Annualised Slope x R^2 of a log-price OLS
    regression over the last `window` trading days (Andreas Clenow,
    "Stocks on the Move"). Same formula/convention as
    ../../Sharpe/momentum_lib.py::compute_clenow / _clenow_window.

    Slope   -- how fast the ETF is trending (annualised).
    R^2     -- how smoothly/consistently it's trending (penalises choppy movers).
    Score   -- ann_slope * r2. Positive & large = steep, smooth uptrend.
    """
    td = CONFIG.ANNUALIZE if trading_days is None else trading_days
    px = series.dropna()
...
def build_ranking(meta: pd.DataFrame, prices: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in meta.iterrows():
        ...
        # Raw Sharpe/Clenow values stored; composites computed post-loop via Z-scores
    ...
    def _blend_composite(df: pd.DataFrame, col6: str, col3: str, w6: float, w3: float,
                         mask: pd.Series | None = None) -> pd.Series:
        """Z-score two windows (over `mask` if given, else the whole df) and
        blend w6/w3, dropping (not rescaling) whichever term is NaN for a
        given row -- same convention throughout this script."""
        z6 = pd.Series(np.nan, index=df.index)
        z3 = pd.Series(np.nan, index=df.index)
        if mask is None:
            z6 = _zscore(df[col6]); z3 = _zscore(df[col3])
        elif mask.sum() > 0:
            z6.loc[mask] = _zscore(df.loc[mask, col6])
            z3.loc[mask] = _zscore(df.loc[mask, col3])
        both  = z6.notna() & z3.notna()
        only6 = z6.notna() & z3.isna()
        only3 = z6.isna()  & z3.notna()
        out = pd.Series(np.nan, index=df.index)
        out.loc[both]  = w6 * z6[both] + w3 * z3[both]
        out.loc[only6] = w6 * z6[only6]
        out.loc[only3] = w3 * z3[only3]
        return out

    # ── Investable composites (reference pool = screen-pass ETFs) ──
    inv_mask = df["SCREEN_PASS"]
    df["SHARPE_COMPOSITE_INV"] = _blend_composite(
        df, "SHARPE_6M", "SHARPE_3M", CONFIG.SHARPE_W6M, CONFIG.SHARPE_W3M, mask=inv_mask)
    df["CLENOW_COMPOSITE_INV"] = _blend_composite(
        df, "CLENOW_6M", "CLENOW_3M", CONFIG.CLENOW_W6M, CONFIG.CLENOW_W3M, mask=inv_mask)

    # Final blended score used for ranking: 0.5*SharpeComposite + 0.5*ClenowComposite
    # (again dropping whichever composite is NaN for a given ETF, not rescaling)
    sc, cc = df["SHARPE_COMPOSITE_INV"], df["CLENOW_COMPOSITE_INV"]
    both_bi  = sc.notna() & cc.notna()
    onlySi   = sc.notna() & cc.isna()
    onlyCi   = sc.isna()  & cc.notna()
    df["_WTD_INV"] = np.nan
    df.loc[both_bi, "_WTD_INV"] = CONFIG.BLEND_W_SHARPE * sc[both_bi] + CONFIG.BLEND_W_CLENOW * cc[both_bi]
    df.loc[onlySi,  "_WTD_INV"] = CONFIG.BLEND_W_SHARPE * sc[onlySi]
    df.loc[onlyCi,  "_WTD_INV"] = CONFIG.BLEND_W_CLENOW * cc[onlyCi]

    # ── Universe composites (reference pool = ALL ETFs; used for display/
    #    fallback sort of ETFs that failed the screen) ──
    df["WTD_SHARPE"] = _blend_composite(df, "SHARPE_6M", "SHARPE_3M", CONFIG.SHARPE_W6M, CONFIG.SHARPE_W3M)
    df["WTD_CLENOW"] = _blend_composite(df, "CLENOW_6M", "CLENOW_3M", CONFIG.CLENOW_W6M, CONFIG.CLENOW_W3M)
    su, cu = df["WTD_SHARPE"], df["WTD_CLENOW"]
    both_bu = su.notna() & cu.notna()
    onlySu  = su.notna() & cu.isna()
    onlyCu  = su.isna()  & cu.notna()
    df["WTD_BLEND"] = np.nan
    df.loc[both_bu, "WTD_BLEND"] = CONFIG.BLEND_W_SHARPE * su[both_bu] + CONFIG.BLEND_W_CLENOW * cu[both_bu]
    df.loc[onlySu,  "WTD_BLEND"] = CONFIG.BLEND_W_SHARPE * su[onlySu]
    df.loc[onlyCu,  "WTD_BLEND"] = CONFIG.BLEND_W_CLENOW * cu[onlyCu]

    # ── Rankings ─────────────────────────────────────────────────
    # Investable rank — blended Sharpe+Clenow composite among screen-pass
    # ETFs only. ETFs that fail the 52-week-high screen get
    # RANK_INVESTABLE = NaN (blank on display), not 0 — they were never
    # ranked at all, since they're not part of the investable population.
    ...
    # Drop intermediate composite working columns
    df = df.drop(columns=["SHARPE_COMPOSITE_INV", "CLENOW_COMPOSITE_INV", "_WTD_INV"], errors="ignore")

    # Sort by investable rank first (passing ETFs at top, best rank first),
    # then by raw Blended score for the remaining (non-investable) ETFs
    df["_sort"] = df["RANK_INVESTABLE"].fillna(9999)
    df = df.sort_values(["_sort", "WTD_BLEND"], ascending=[True, False]).drop(columns="_sort").reset_index(drop=True)
...
def print_summary(df, regime, allocation):
    ...
    print(f"\n  RANKING  (investable rank = Sharpe+Clenow blend, scored among {inv_count} ETFs passing abs filter)")
    print(f"  {'InvRk':>5} {'Ticker':<14} {'ETF Name':<30} "
          f"{'Blend':>8} {'Sharpe6M':>9} {'Sharpe3M':>9} {'Clenow6M':>9} {'Clenow3M':>9} "
          f"{'Screen':>7}")
    print("  " + "-" * 105)
    ...
    for _, r2 in df.head(5).iterrows():
        ...
        print(f"  {inv_rk:>5} {r2['TICKER']:<14} "
              f"{str(r2['ETF_NAME'])[:29]:<30} "
              f"{f(r2['WTD_BLEND']):>8} {f(r2['SHARPE_6M']):>9} {f(r2['SHARPE_3M']):>9} "
              f"{f(r2['CLENOW_6M']):>9} {f(r2['CLENOW_3M']):>9} "
              f"{screen:>7}")

    print(f"\n  Universe={len(df)}  Investable (both screens pass)={inv_count}  "
          f"Screened out={len(df)-inv_count}  Valid Blend Score={df['WTD_BLEND'].notna().sum()}")
    print("=" * W)
...
HOLDINGS_LOG_FILE = "holdings_log_blend.json"
...
def save_excel(df, regime, allocation, out_path, prev_entry=None, changes=None, log=None):
    ...
    c.value     = ("ETF Momentum Ranking  |  "
                   ...
                   "Step 2: Score (Sharpe + Clenow Blend)  ->  "
    ...
    KEYS = [
        ...
        "EMA_100",
        "EOM_PCT",
        "WTD_BLEND", "SHARPE_6M", "SHARPE_3M", "CLENOW_6M", "CLENOW_3M",
        "SCREEN_PASS",
    ]
