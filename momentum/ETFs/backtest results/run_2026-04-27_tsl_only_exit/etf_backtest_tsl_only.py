"""
ETF Momentum Backtest — TSL-Only Exit Variant
================================================
Same regime / scoring / screen as Run 1, but positions are NEVER sold
for rank reasons. The ONLY exit triggers are:

  1. Daily 10% Trailing Stop Loss breach
  2. Regime turns BEAR (full cash)

New entries fill empty slots every Monday from the Top-N ranking.
Entry sizing: 1/5th of current total portfolio value.

This tests whether pure TSL-driven exits (no rank churn) outperform
the monthly full-reset approach.
"""
...
OUT_DIR = r"C:\Users\ameet\Documents\Github\code\momentum\ETFs\backtest results\run_2026-04-27_tsl_only_exit"
...
def run_backtest() -> dict:
    ...
    print(f"Strategy: TSL-ONLY EXIT | Weekly Monday check | 10% TSL | "
    ...
    for idx, monday in enumerate(mondays):
        ...
        # ── NO rank-based selling ──────────────────────────────
        # Positions are held until TSL breach or BEAR regime.
        # Only trim if we exceed max slots (e.g. BULL->PARTIAL transition).
        while len(portfolio) > regime_slots:
            # Sell the held position with the lowest current WT_SHARPE
            worst_t = None
    ...
    print("\n" + "="*55)
    print(f"  BACKTEST: TSL-ONLY EXIT  |  10% TSL  |  Weekly Monday")
    print("="*55)
    ...
    ax.plot(res["equity"], label="TSL-Only Exit (Weekly Mon, 10% TSL)",
    ...
    ax.fill_between(res.index, res["equity"], initial, alpha=0.07, color="darkcyan")
    ax.set_title("ETF Momentum — TSL-Only Exit | Weekly Monday Check | 10% TSL | Run 1 Regime")
