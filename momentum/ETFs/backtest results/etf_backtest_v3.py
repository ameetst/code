"""
ETF Momentum Backtest Engine -- v3 (Weekly + 3-State Regime + 3-Window Scoring)
================================================================================
Hybrid strategy combining v1's proven risk management with weekly cadence:

  Scoring   : 0.4*Sharpe_1M + 0.4*Sharpe_3M + 0.2*Sharpe_6M  (all-positive composite)
              If 6M unavailable (3-6M data): 0.4*Sh1M + 0.4*Sh3M
              If 1M or 3M unavailable (<3M data): ETF excluded entirely
  Screen    : NAV within 25% of 52-week high
  Regime    : 3-state, evaluated weekly (first trading day of week):
                BULL    : EMA50 > EMA100 AND Price > EMA50  -> 5 slots
                PARTIAL : Price > EMA100 (not BULL)         -> 3 slots (trim to 3 if over)
                BEAR    : Price <= EMA100                   -> 0 slots, exit ALL -> 100% cash
  Rebalance : Weekly selective exit -- only exit on TSL/rank/regime-BEAR
              In PARTIAL: trim to top-3 by rank if holding more than 3
              Fill empty slots up to max_slots in BULL/PARTIAL
  Exit rules:
    1. Daily  TSL: 10% trailing stop from running peak -> immediate exit that day
    2. Weekly rank: exit if rank > RANK_RETENTION (20) among screen-pass ETFs
    3. Regime BEAR: exit ALL positions on next Monday -> 100% cash
    4. Regime PARTIAL: trim lowest-ranked positions down to 3 slots on Monday
...
Comparison:
  Loads v1 equity CSV and produces a 3-way comparison chart (v1 vs v3 vs Nifty 500).

Usage:
  python etf_backtest_v3.py
"""
...
class CONFIG:
    ...
    TRADE_LOG_FILE   = str(_BASE / "v3_backtest_trade_log.csv")
    EQUITY_LOG_FILE  = str(_BASE / "v3_backtest_equity.csv")
    CHART_FILE       = str(_BASE / "v3_equity_curve.png")
    COMPARE_CHART    = str(_BASE / "v3_comparison_equity_curve.png")
...
# =========================================================
# SECTOR CLASSIFICATION (same as etf_backtest.py)
# =========================================================
_SECTOR_RULES = [
...
# =========================================================
# SCORING -- v3 formula: 0.4*Sh1M + 0.4*Sh3M + 0.2*Sh6M
# =========================================================
...
def compute_wtd_sharpe(sh1: float, sh3: float, sh6: float) -> float:
    """
    v3 composite score: 0.4*Sh1M + 0.4*Sh3M + 0.2*Sh6M (all positive weights).
    - Excludes ETF entirely if 1M or 3M is NaN (< ~3 months data).
    - Drops 6M term if NaN (3-6 months data), rescales remaining weights to sum to 1:
        -> 0.5*Sh1M + 0.5*Sh3M when 6M unavailable.
    """
    if np.isnan(sh1) or np.isnan(sh3):
        return np.nan
    if np.isnan(sh6):
        # Rescale 1M and 3M weights to sum to 1 (0.4/0.8 each = 0.5 each)
...
def rank_universe(meta: pd.DataFrame, prices: pd.DataFrame,
    ...
    # Rank only screen-pass ETFs with a valid score (descending)
...
def evaluate_regime(regime_s: pd.Series, regime_ema50: pd.Series,
                    regime_ema100: pd.Series, as_of: pd.Timestamp) -> tuple[str, int]:
    """
    3-state regime (v1 logic):
      BULL    : EMA50 > EMA100 AND Price > EMA50  -> 5 slots
    ...
    if as_of not in regime_s.index:
        return "BULL", CONFIG.TOP_N   # default
...
def _current_sector_counts(portfolio: dict) -> dict:
    ...
    for pos in portfolio.values():
        counts[pos.get("sector", "OTHER")] = counts.get(pos.get("sector", "OTHER"), 0) + 1
    return counts
...
# =========================================================
# MAIN v3 BACKTEST ENGINE
# =========================================================
def run_backtest_v3() -> dict:
    """
    Full v3 backtest:
      - Weekly selective rebalance (first trading day of week)
      - Daily 10% TSL check
      - 3-state BULL/PARTIAL/BEAR regime (v1 logic, evaluated weekly)
      - BEAR: exit ALL positions on Monday -> 100% cash
      - PARTIAL: trim to top-3 by rank on Monday
      - BULL: fill up to 5 slots from ranked investable universe
    """
    ...
    weekly_rank_df = pd.DataFrame()

    print(f"Starting v3 backtest: {date_list[0].date()} -> {date_list[-1].date()}")
    print(f"TSL: {CONFIG.TSL_THRESHOLD*100:.0f}%  |  Regime: 3-state BULL/PARTIAL/BEAR"
          f"  |  Rank retention: top {CONFIG.RANK_RETENTION}  |  RF: 6% p.a.\n")
    ...
    for d in all_dates:
        ...
        if is_rebalance:
            ...
            # ---- BEAR: exit ALL positions unconditionally ----
            if regime_label == "BEAR":
                for t in list(portfolio.keys()):
                    pos    = portfolio[t]
                    p_exit = prices.loc[d, t] if (t in prices.columns
                                                   and not pd.isna(prices.loc[d, t])) \
                             else pos["entry_price"]
                    cash = _sell_position(t, pos, p_exit, d,
                                         "BEAR_REGIME_EXIT", regime_label,
                                         cash, trade_log)
                portfolio = {}

            else:
                # ---- Rank-based selective exits (BULL and PARTIAL) ----
                for t in list(portfolio.keys()):
                    ...
                    # rank == 0 -> screened out or no score -> exit
                    if rank == 0 or rank > CONFIG.RANK_RETENTION:
                        ...
                        cash = _sell_position(t, pos, p_exit, d,
                                              f"RANK_EXIT (rank={rank})", regime_label,
                                              cash, trade_log)
                        del portfolio[t]

                # ---- PARTIAL: trim to TOP_N_PARTIAL ----
                if regime_label == "PARTIAL" and len(portfolio) > CONFIG.TOP_N_PARTIAL:
                    # Rank holdings and exit the lowest-ranked ones first
                    held = []
                    for t, pos in portfolio.items():
                        row  = weekly_rank_df[weekly_rank_df["TICKER"] == t] \
                               if not weekly_rank_df.empty else pd.DataFrame()
                        rank = int(row["RANK"].iloc[0]) if not row.empty else 9999
                        held.append((rank, t, pos))
                    held.sort(key=lambda x: x[0])  # best rank first

                    to_exit = held[CONFIG.TOP_N_PARTIAL:]  # trim the tail
                    for rank, t, pos in to_exit:
                        p_exit = prices.loc[d, t] if (t in prices.columns
                                                       and not pd.isna(prices.loc[d, t])) \
                                 else pos["entry_price"]
                        cash = _sell_position(t, pos, p_exit, d,
                                              f"PARTIAL_TRIM (rank={rank})", regime_label,
                                              cash, trade_log)
                        del portfolio[t]

                # ---- Fill empty slots ----
                if len(portfolio) < active_slots and not weekly_rank_df.empty:
                    # Slot size = equal share of total portfolio value across TOP_N slots
                    total_val = cash + _portfolio_value(portfolio, prices, d)
                    slot_size = total_val / CONFIG.TOP_N
    ...
    bear_exits = sells[sells["REASON"].str.startswith("BEAR")]  if len(sells) else pd.DataFrame()
    part_trim  = sells[sells["REASON"].str.startswith("PART")]  if len(sells) else pd.DataFrame()
    ...
    print(f"  Rank exits (>20)     : {len(rank_exits)}")
    print(f"  BEAR regime exits    : {len(bear_exits)}")
    print(f"  PARTIAL trim exits   : {len(part_trim)}")
    if len(sells) > 0:
    ...
    print("\n" + "=" * 54)
    print(f"  v3 BACKTEST RESULTS  (10% TSL | BULL/PARTIAL/BEAR | Weekly)")
    print("=" * 54)
    ...
    _plot_single(eq["equity"], bench, initial,
                 title="ETF Momentum v3 -- Weekly | 10% TSL | BULL/PARTIAL/BEAR Regime",
                 filepath=CONFIG.CHART_FILE)

    _plot_comparison(eq["equity"], bench, initial,
                     v3_cagr=cagr, v3_dd=max_dd, v3_sharpe=sharpe,
...
def _plot_single(eq_series: pd.Series, bench, initial: float,
    ...
    ax.plot(eq_series, label="Strategy v3 (Weekly, 10% TSL, 3-state Regime)",
    ...
    ax.fill_between(eq_series.index, eq_series, initial, alpha=0.07, color="#9C27B0")
    ...
    plt.close(fig)
    print(f"\nv3 equity curve -> {filepath}")


def _plot_comparison(eq_v3: pd.Series, bench, initial: float,
                     v3_cagr: float, v3_dd: float, v3_sharpe: float,
                     b_cagr, b_dd, b_sharpe):
    """
    3-way comparison: v1 (Monthly) vs v3 (Weekly hybrid) vs Nifty 500.
    ...
    # Equity panel
    v3_seg  = eq_v3.loc[start:end]
    v3_norm = (v3_seg / v3_seg.iloc[0]) * initial
    ax_eq.plot(v3_norm,
               label=f"v3 Weekly Hybrid | CAGR {v3_cagr:.1%} | DD {v3_dd:.1%} | Sharpe {v3_sharpe:.2f}",
               linewidth=2.2, color="#9C27B0", zorder=3)
    ...
    if bench is not None:
        b_seg  = bench.reindex(v3_seg.index, method="ffill")
    ...
    ax_eq.set_title("ETF Momentum Strategy -- v1 (Monthly) vs v3 (Weekly Hybrid) vs Nifty 500",
    ...
    # Drawdown panel
    v3_dd_s = (v3_norm - v3_norm.cummax()) / v3_norm.cummax() * 100
    ax_dd.fill_between(v3_dd_s.index, v3_dd_s, 0,
                       alpha=0.45, color="#9C27B0", label="v3 Drawdown")
    if v1_eq is not None:
    ...
    for (r, c), cell in table.get_celld().items():
        if r == 0:
            ...
            cell.set_facecolor("#F3E5F5")   # purple tint for v3
        elif c == 2:
