"""
ETF Momentum Backtest Engine  -  Run 1 (Best Performing Config)
===============================================================
Regime Filter (evaluated at start of each month, using prev month-end close):
  BULL    : EMA50 > EMA100  AND  Price > EMA50  ->  5 slots
  PARTIAL : Price > EMA100  (but not BULL)       ->  3 slots, remainder in cash
  BEAR    : Price <= EMA100                       ->  0 slots, full cash

Rebalance : Monthly (first trading day of month) - exit all, rank, enter fresh
Monitoring : Daily TSL (10% trailing stop loss)
Cash       : 2% p.a. on idle capital
"""
...
def r2_score(series: pd.Series, window: int) -> float:
    """R-squared from log-linear regression (trend consistency)."""
    clean = series.dropna()
    if len(clean) < window:
        return np.nan
...
def run_backtest(ranking_metric: str = "WT_SHARPE") -> dict:
    ...
    Regime (Run 1):
      ...
      PARTIAL : Price > EMA100                       -> TOP_N_PARTIAL slots
      BEAR    : Price <= EMA100                      -> 0 slots (full cash)
    """
    ...
    for idx, start_date in enumerate(mondays):
        ...
        # ── Regime (Run 1 logic) ───────────────────────────
        regime_slots = CONFIG.TOP_N   # default BULL if no data
        regime_label = "BULL"
        if regime_series is not None:
            s_reg = regime_series.loc[:prev_trading_day].dropna()
        ...
        if num_to_buy > 0:
            slot_size = cash / CONFIG.TOP_N   # equal-weight based on TOP_N slots
            for r in selected:
                t       = r["TICKER"]
    ...
    print(f"  Weekly sells  : {len(sells[sells['REASON']=='WEEKLY_REBALANCE'])}" if len(sells) else "")
    ...
    print("\n" + "="*45)
    print(f"  BACKTEST RESULTS  ({CONFIG.TSL_THRESHOLD*100:.0f}% TSL  |  Run 1 Regime)")
    print("="*45)
    ...
    ax.plot(res["equity"], label=f"Strategy ({ranking_metric}, 10% TSL)",
    ...
    ax.set_title("ETF Momentum Backtest - Run 1 Regime | 10% TSL | Weekly Mon Rebal | Daily TSL Monitor")
