"""
ETF Momentum Backtest — EMA50 Hold Regime
==========================================
Two-state regime based on Nifty 500 vs its 50-day EMA:
  ...
  HOLD  (price <= EMA50):
    ...
    - Existing positions stay unless:
        ...
        b) NAV falls >25% below 52-week high (screen failure, checked monthly)
    - Empty slots remain as cash (2% p.a.)

Transitions:
  BULL → HOLD : Existing positions carry forward; apply HOLD rules next month
  HOLD → BULL : Full monthly rebalance at transition, fill all 5 slots fresh
...
OUT_DIR = r"C:\Users\ameet\Documents\Github\code\momentum\ETFs\backtest results\run_2026-05-18_ema50_hold"
...
class CONFIG:
    ...
    MAX_DD_FROM_HIGH = 0.25          # 25% screen threshold (HOLD exit trigger)
    ...
    TREND_FAST_EMA   = 50            # Only EMA50 used for regime
...
# =========================================================
# HELPER: compute WT_SHARPE ranked list from history
# =========================================================
...
# =========================================================
# HELPER: sell a position, return net proceeds
# =========================================================
...
def _buy(portfolio, t, name, sector, p_entry, slot_size, date, regime_label, trade_log):
    ...
    return slot_size + CONFIG.TRADE_COST_FIXED   # amount deducted from cash


# =========================================================
# BACKTEST ENGINE — EMA50 Hold Regime
# =========================================================
def run_backtest() -> dict:
    ...
    print(f"Regime: EMA50 BULL/HOLD | TSL: {CONFIG.TSL_THRESHOLD:.0%} | "
    ...
    for m_idx, (start_date, end_date) in enumerate(zip(first_days, last_days)):
        ...
        if regime_series is not None:
            ...
            if len(s_reg) >= CONFIG.TREND_FAST_EMA:
                p_now  = float(s_reg.iloc[-1])
                ema50  = float(s_reg.ewm(span=CONFIG.TREND_FAST_EMA, adjust=False).mean().iloc[-1])
                is_bull = p_now > ema50
                regime_label = "BULL" if is_bull else "HOLD"

        # ── Compute full ranked list (used by both BULL and HOLD) ─
        ranked = rank_etfs(meta, hist_prices)

        # =========================================================
        # BULL: Full monthly rebalance (sell all → re-enter top 5)
        # =========================================================
        if is_bull:
        ...
        # =========================================================
        # HOLD: No new entries — only exit weak positions
        # Exit conditions: TSL (intra-month daily) OR screen failure (>25% from 52W high)
        # =========================================================
        else:
            # Check each held position against the 52W screen at month start
            # Build a set of tickers that pass the screen right now
            screen_pass = {r["TICKER"] for r in ranked}  # ranked already filters screen

            for t in list(portfolio.keys()):
                if t not in screen_pass:
                    # Failed 52W high screen — exit immediately
                    p_exit = prices.loc[start_date, t] if t in prices.columns else np.nan
                    if pd.isna(p_exit):
        ...
        # =========================================================
        # DAILY monitoring: TSL + cash interest (both BULL and HOLD)
        # =========================================================
    ...
    rebal_exits  = sells[sells["REASON"] == "BULL_REBALANCE"]       if len(sells) else pd.DataFrame()

    # Count BULL and HOLD months
    bull_months = sum(1 for m_idx, (s, e) in enumerate(zip(first_days, last_days))
                      if m_idx > 0 and regime_series is not None and
                      len(regime_series.loc[:last_days[m_idx-1]].dropna()) >= CONFIG.TREND_FAST_EMA and
                      float(regime_series.loc[:last_days[m_idx-1]].dropna().iloc[-1]) >
                      float(regime_series.loc[:last_days[m_idx-1]].dropna()
                            .ewm(span=CONFIG.TREND_FAST_EMA, adjust=False).mean().iloc[-1]))
    total_months = len(first_days) - 1

    print(f"\nTrade Log -> {CONFIG.TRADE_LOG_FILE}")
    print(f"  Total trades       : {len(tlog)}")
    print(f"  Total buys         : {len(tlog[tlog['TYPE']=='BUY'])}" if len(tlog) else "")
    print(f"  Rebalance sells    : {len(rebal_exits)}")
    print(f"  Screen exits (HOLD): {len(screen_exits)}")
    print(f"  TSL hits           : {len(tsl_hits)}")
    print(f"  BULL months        : {bull_months} / {total_months}")
    if len(sells) > 0:
    ...
    print("\n" + "="*55)
    print(f"  EMA50 HOLD REGIME  |  10% TSL  |  Monthly (BULL only)")
    print("="*55)
    ...
    ax.plot(res["equity"], label="EMA50 Hold Regime (10% TSL)",
            linewidth=2, color="steelblue")
    if run1_ok and run1_eq is not None:
        ax.plot(run1_eq, label="Run 1 — EMA100 PARTIAL/BEAR (10% TSL)",
                linewidth=1.5, color="darkorchid", linestyle="--", alpha=0.85)
    if bench_ok:
        ax.plot(bench, label="Benchmark (Nifty 500)", linewidth=1.5,
                alpha=0.65, color="darkorange", linestyle="--")
    ...
    ax.set_title("ETF Momentum — EMA50 Hold Regime | Monthly Rebalance (BULL only) | 10% TSL")
