"""
ETF Dual Momentum + Clenow + 3W-STRICT Sharpe + Tiered Regime Filter
====================================================================
VARIANT of etf_momentum_ranking.py -- the only change is the ranking
composite (Step 2 below). Everything else (screen, regime, rebalance,
exits, allocation) is identical to the live script.

Composite change (validated via backtest + live-ranking comparison,
see "backtest results/etf_backtest_v9_multiwindow_sharpe.py" and
"backtest results/compare_live_rankings_4w3w_strict.py"):
  OLD (live)  : Z(Sharpe_6M)*0.5 + Z(Sharpe_3M)*0.5
  NEW (3W-STRICT) : mean(Z(Sharpe_12M), Z(Sharpe_6M), Z(Sharpe_3M)),
                     equal-weighted, but ONLY if all three windows are
                     valid for that ETF -- otherwise no composite score
                     (falls to the bottom of RANK_INVESTABLE). This
                     filters out ETFs without a full 12-month trading
                     history rather than scoring them on whatever
                     partial data is available.
  Backtest result (2020-04 -> 2026-07): CAGR 21.11% vs 16.51% live,
  Max Drawdown -19.97% vs -20.82% live, Sharpe 1.67 vs 1.36 live.
  Effect held (not purely a post-2023 artifact) in both the pre-2023
  and 2023+ sub-periods, and specifically avoided the live script's
  -1.63% year in the 2022 chop (3W-STRICT: +3.53%).

Isolated state: this script uses its OWN output/log/config files
(etf_rankings_3wstrict.xlsx, holdings_log_3wstrict.json,
strategy_config_3wstrict.json) so running it never touches the live
script's production state. It reads the SAME ETF.xlsx price snapshot.
...
Scoring pipeline (in correct order):
  ...
  Step 2 - SCORE   : 3W-STRICT Sharpe composite (12M/6M/3M, Z-scored, all required)
                     on ALL ETFs for reference.
...
# =========================================================
# CONFIG  <- loaded from strategy_config_3wstrict.json if available,
#            otherwise uses hardcoded defaults below.
#            (Separate config file from the live script -- isolated state.)
# =========================================================
_SCRIPT_DIR = Path(__file__).resolve().parent
_CONFIG_FILE = "strategy_config_3wstrict.json"
...
class CONFIG:
    INPUT_FILE  = "ETF.xlsx"
    OUTPUT_FILE = "etf_rankings_3wstrict.xlsx"
...
HOLDINGS_LOG_FILE = "holdings_log_3wstrict.json"
